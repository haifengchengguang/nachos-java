 ...
 */
/**
 <tt>Interrupt</tt>类模拟了低级别的中断硬件。该
 * 硬件提供一个方法（<tt>setStatus()</tt>）来启用或禁用
 * 中断。
 *
 * <p>
 * 为了模拟硬件，我们需要跟踪所有待处理的
 * 硬件设备将导致的中断，以及它们应该何时发生
 *发生。
 *
 * <p>
 * 这个模块还记录了模拟的时间。时间只在以下情况下前进
 *发生以下情况。
 * <ul>
 * <li>中断被启用，而之前它们是被禁用的
 <li>一条MIPS指令被执行
 ...
 *
 * <p>
 * 因此，与真正的硬件不同，中断（包括时间片上下文
 *开关）不能仅仅发生在代码中的任何地方，在那里中断被启用
 * 启用，而只是在代码中模拟时间的那些地方
 * 进展的地方（这样，硬件模拟就有时间来调用
 * 中断处理程序）。
 *
 * <p>
 * 这意味着，不正确的同步代码可能在这个硬件上运行良好
 * 仿真（即使有随机的时间片），但它在真实的
 *硬件上就不行了。但是，即使Nachos不能总是检测到你的程序在什么时候会失败
 * 会在现实生活中失败，你仍然应该编写正确的同步代码。
 */
public final class Interrupt {
	...
	//不同情况下前进的始终数目不同
	//inKernelMode==true指的是在关中断开中断后tick，此刻前进10个系统时钟
	//inKernelMode==false指的是在执行了一条用户程序指令之后tick，此时前进1个系统时钟
	if (inKernelMode) {
	...
	//从pending容器中取出终端，并且将已经到了的触发事件的中断触发
