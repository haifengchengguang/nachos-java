...
public class AutoGrader {
    ...
    //在 run()函数运行内核的过程中会根据 nachos.conf 内的配置不同而执
	//行不同的内核函数。在原始的配置中,内核是 nachos.threads.ThreadedKernel,
	//因此会分别执行 nachos.threads 包下的 ThreadedKernel 类的 selfTest()、run()、
	//terminate()函数,最终在 ThreadedKernel 类的 selfTest()函数中运行 PingTest()函
	//数,会输出前面展示的配置成功后的那些输出信息。
    void run() {
	kernel.selfTest();
	kernel.run();
	kernel.terminate();
